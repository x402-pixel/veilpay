...
import { ledger, } from '../../contract/src/managed/veilpay3/contract/index.js';
...
const deployedAddress = () => {
    ...
    if (envAddr)
        return envAddr.trim();
    if (fs.existsSync(ADDRESS_FILE_V3))
        return fs.readFileSync(ADDRESS_FILE_V3, 'utf8').trim();
    return null;
};
const STATUS_NAMES = ['ACTIVE', 'PAID', 'SETTLED', 'CANCELLED'];
const TYPE_NAMES = ['standard', 'multipay', 'donation'];
...
const invoiceJson = (invoiceId, state) => JSON.stringify({
...
const run = async (argv) => {
    ...
    const logger = await createLogger(path.resolve(STATE_DIR, '..', 'logs', 'preprod', `gateway3-tx-${new Date().toISOString().slice(0, 10)}.log`));
    ...
    try {
        ...
        switch (command) {
            case 'issue': {
                ...
                const invoiceType = (rest[2] ?? 'standard');
                if (!TYPE_NAMES.includes(invoiceType))
                ...
                console.log(JSON.stringify({ ...issued, explorer: `${EXPLORER}/contracts/0x${address}` }, null, 2));
                break;
            }
            case 'pay': {
                ...
                const state = (await providers.privateStateProvider.get('veilPay3PrivateState'));
...
//# sourceMappingURL=gateway-cli3.js.map