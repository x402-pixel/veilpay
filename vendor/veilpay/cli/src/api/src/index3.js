...
import { veilPay3PrivateStateKey, } from './common-types.js';
import * as utils from './utils/index.js';
import { deployContract, findDeployedContract, } from '@midnight-ntwrk/midnight-js-contracts';
import { combineLatest, map, tap, from } from 'rxjs';
import { toHex } from '@midnight-ntwrk/midnight-js-utils';
import { createVeilPay3PrivateState, withInvoiceOpening3, } from '../../contract/src/witnesses3.js';
export const CompiledVeilPay3ContractContract = CompiledContract.make('VeilPay3', (VeilPay3Generated.Contract)).pipe(CompiledContract.withWitnesses(witnesses3Module.witnesses3), CompiledContract.withCompiledFileAssets('./managed/veilpay3'));
...
const INVOICE_TYPE_NAMES = ['standard', 'multipay', 'donation'];
export const invoiceTypeName = (code) => INVOICE_TYPE_NAMES[code] ?? 'standard';
...
export class VeilPay3API {
    ...
    constructor(deployedContract, providers, logger) {
        ...
        this.state$ = combineLatest([
            providers.publicDataProvider
                ...
                .pipe(map((contractState) => VeilPay3Generated.ledger(contractState.data)), tap((ledgerState) => logger?.trace({ ledgerSequence: ledgerState.sequence.toString() }))),
            from(providers.privateStateProvider.get(veilPay3PrivateStateKey)),
        ], (l, privateState) => {
    ...
    state$;
    async privateState() {
        const existing = (await this.providers.privateStateProvider.get(veilPay3PrivateStateKey));
    ...
    async stageOpening(invoiceId, opening) {
        const state = await this.privateState();
        await this.providers.privateStateProvider.set(veilPay3PrivateStateKey, withInvoiceOpening3(state, invoiceId, opening));
    }
    ...
    async issueInvoice(args) {
        ...
        const txData = await this.deployedContract.callTx.issueInvoice(opening.amount, opening.tokenColor, opening.merchantCoinPk, INVOICE_TYPE_CODES[args.invoiceType], args.expiresAt);
        const invoiceId = txData.private.result ?? nextId;
        return {
    ...
    async settleStandard(invoiceId, opening, coin) {
    ...
    async settleMultiPayment(invoiceId, opening, coin) {
        await this.stageOpening(invoiceId, opening);
    ...
    async isSettled(invoiceId) {
        ...
        return (status === VeilPay3Generated.InvoiceStatus.PAID ||
    ...
    static async getPrivateState(providers, contractAddress) {
        ...
        const existing = (await providers.privateStateProvider.get(veilPay3PrivateStateKey));
        return existing ?? createVeilPay3PrivateState(utils.randomBytes(32), utils.randomBytes(32));
    }
}
